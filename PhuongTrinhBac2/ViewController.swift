...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func GiaiPT(_ sender: Any) {
        ...
        if ((txtA.text?.isEmpty)! || (txtB.text?.isEmpty)! || (txtC.text?.isEmpty)!){
            labelKQ1.text = "Bạn phải nhập vào giá trị!!!"
        }
            ...
            if(a==0 && b==0 && c==0)
            {
                labelKQ1.text = "Phương trình vô số nghiệm"
            }
                
                
            else if( a==0 ){
                labelKQ1.text = "Phương trình có nghiệm kép x = \(-c/b)"
            }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if filtered == string {
            ...
            if string == "." { // kiem tra so thap phan
